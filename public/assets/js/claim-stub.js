$(document).ready(function(){
    ...
    $(document).on('change','#ftr_selby',function(){
        ...
        if (ftr_selby == 'apptype') {
            $('.ftr_taxyr').attr('hidden',true);
            $('.ftr_apptype').attr('hidden',false);
            $('#ftr_val').val('');
            $("#ftr_val").inputmask({
                "mask": "9999"
            });
        } else if (ftr_selby == 'taxyear') {
    ...
    var tblclaimstub = $('#claim-stub').DataTable({
        'dom': '<"wrapper"Bfritp>',
        'order': [
            [0, "desc"]
        ],
    ...
    $(document).on('submit','#ftr_claimstub',function(e){
        e.preventDefault();
        $('#frm_submit').attr('hidden',true);
        $('#frm_loading').attr('hidden',false);
        var arr = $(this).serialize();
      
        axios.post(base_url + "/ibcas/claim-stub/filter", arr)
        .then(function(response) {
          
            var data = response.data.TableContent;
            console.log(data);
            $('#claim-stub').dataTable().fnDestroy();

            var CLAIMSTUB = $('#claim-stub').DataTable({
                data: data,
                ...
                columns: [
                    { data: 'name' },
                    { data: 'tax_effectivity_year' },
                    {
                        "data": function(data, type, row, meta) {
                            return `${dateformat(data.first_quarter_date)}`;
                        }
                    },
                    { 
                        "data": function(data, type, row, meta) {
                        return `${dateformat(data.second_quarter_date)}`;
                        } 
                    },
                    {
                        "data": function(data, type, row, meta) {
                            return `${dateformat(data.third_quarter_date)}`;
                            }
                    },
                    {
                        "data": function(data, type, row, meta) {
                            return `${dateformat(data.fourth_quarter_date)}`;
                            }
                    },
                    {
                        "data": function(data, type, row, meta) {
                            return `
                            <a data-toggle="tooltip" title="View schedule" href="${base_url}/ibcas/claim-stub/view/${data.id}" target="_blank" class="btn btn-sm btn-soft-primary btn-view-details"><i class="mdi mdi-eye-outline"></i></a>
                            <a data-toggle="tooltip" title="Update schedule" href="${base_url}/ibcas/claim-stub/edit/${data.id}" target="_blank" class="btn btn-sm btn-soft-info btn-edit"><i class="mdi mdi-pencil-outline"></i></a>
                            <a  data-toggle="tooltip" title="Delete schedule" href="javascript:void(0);" class="btn btn-sm btn-soft-danger btn-delete delete"  data-year="${data.year}" data-id="${data.id}"><i class="mdi mdi-delete-outline"></i></a>
                          `
                        }
                    },
                ],                
