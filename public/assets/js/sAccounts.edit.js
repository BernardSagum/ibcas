  ...
  $(document).on('submit', '#frm-addSubAccount', function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            showOverlay('Updating Sub-Account......')
...
$('#classifications-table tbody').on('click', 'tr', function() {
  ...
  //     var arr = $(this).serialize() + '&type=' + encodeURIComponent(joinedValues);
...
$(document).on('click', '.delete-section', function(e) {
    ...
    if($(this).attr('data-id') == ''){
        ...
        Swal.fire({
            title: 'Delete Particular and Classification?',
          text: "",
          icon: 'question',
              showCancelButton: true,
              confirmButtonColor: '#3085d6',
              ...
              if (result.isConfirmed) {
                ...
                axios.get(base_url + "/ibcas/sAccounts/deleteClassPart/"+part_id)
                .then(function(response) {
                    
                    if (response.data.status == 200) {
                      $('#tstsuccess strong').text(response.data.message);
                        toastsuccess.show();
                        elem.closest('.dynamic-section').prev('.dynamic-section').remove();
                        elem.closest('.dynamic-section').remove();
                    } else {
                      $('#tsterror strong').text(response.data.message);
                    toasterror.show();
                    }
...
function getSubAccountDetails(id){
    ...
    .then(function(response) {
        if (response.data.status == 200) {
            ...
            document.getElementById('effectivity_year').value = response.data.SubAccountInfo[0].effectivity_year;
            document.getElementById('parentCodelabel').value = response.data.SubAccountInfo[0].acc_code;
            document.getElementById('account_id').value = response.data.SubAccountInfo[0].acc_id;
            
            // document.getElementById('account_id').value = response.data.SubAccountInfo[0].acc_id;
            document.getElementById('subcode').value = response.data.SubAccountInfo[0].sub_code;
            $('#subcode').attr('disabled', false);
            document.getElementById('subcodedesc').value = response.data.SubAccountInfo[0].sub_desc;
            document.getElementById('remarks').value = response.data.SubAccountInfo[0].remarks;

            // Your input string
                const inputString = response.data.SubAccountInfo[0].sub_type;

                // Split the string into an array
                const values = inputString.split(',');

                // Iterate over the checkboxes and check them if they match the values from the array
                values.forEach(value => {
                    const checkbox = $(`.optcbx[data-val="${value}"]`);
                    if (checkbox.length) {
                ...
                .then(function(response) {
                    if (response.data.status == 200) {
                        ...
                        arr.forEach(function(item) {
                            var newSection = `
                                <div class="dont-inputcount">
                                <div class="row mb-3 dynamic-section">
                                <input type="hidden" name="secCounter[]" value="${item.id}">
                                    <div class="col">
                                        <label for="searchParticular${item.id}" class="form-label">Search Particular:</label>
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="searchParticular[]" placeholder="Enter particular" readonly value="${item.par_name}">
                                            
                                        </div>
                                    </div>
                                    <div class="col">
                                        <label for="searchClassification${item.id}" class="form-label">Search Classification:</label>
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="searchClassification[]" placeholder="Enter classification" readonly value="${item.className}">
                                ...
                                <div class="row mb-3 dynamic-section">
                                    <div class="col">
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="checkbox" name="dynamicCheckboxN[]" value="N" ${item.new === 'Y' ? 'checked' : ''}>
                                            <label class="form-check-label checkbox-label">N</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="checkbox" name="dynamicCheckboxR[]" value="R" ${item.renewal === 'Y' ? 'checked' : ''}>
                                            <label class="form-check-label checkbox-label">R</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="checkbox" name="dynamicCheckboxC[]" value="C" ${item.closure === 'Y' ? 'checked' : ''}>
                                            <label class="form-check-label checkbox-label">C</label>
                                        </div>
                                        <a href="#" class="btn btn-sm btn-danger delete-section" data-id="${item.id}">Delete</a>
                                    </div>
                                </div>
                                </div>`;
                            $('#dynamic-content').append(newSection);
