$(document).ready(function(){
    ...
    var id = document.getElementById('col_id').value;
    // setTimeout(function() {
        loadViewData(id);
    //   }, 2000);
    ...
    function loadViewData(id){
        showOverlay('Loading Collector Information')
        axios.get(base_url + "/ibcas/collectors/get-view-data/"+id)
        .then(function(response) {
            
            // console.log(response.data.recdata[0].name);
            document.getElementById('user_id').value = response.data.TableContent[0].user_id;
            document.getElementById('user_label').value = response.data.TableContent[0].collectorname;
            document.getElementById('address').value = response.data.TableContent[0].address;
            document.getElementById('barangay_id').value = response.data.TableContent[0].barangay_id;
            document.getElementById('contact_number').value = response.data.TableContent[0].contact_number;
            document.getElementById('email').value = response.data.TableContent[0].email;
            document.getElementById('tin_no').value = response.data.TableContent[0].tin_no;
            document.getElementById('position_id').value = response.data.TableContent[0].position_id;
            document.getElementById('accountable_officer_id').value = response.data.TableContent[0].accountable_officer_id;
            document.getElementById('officerlabel').value = response.data.TableContent[0].accountable_person;
            // document.getElementById('officerlabel').value = response.data.TableContent[0].accountable_person;
            var deputized_collector = response.data.TableContent[0].deputized_collector;
            var field_collector = response.data.TableContent[0].field_collector;

            if(deputized_collector == 'Y'){
                $('#deputized_collector').prop('checked',true);
            }
            if(field_collector == 'Y'){
                $('#field_collector').prop('checked',true);
            }
    ...
    $(document).on('submit','#frm_edit_collector',function(e){
        e.preventDefault();
